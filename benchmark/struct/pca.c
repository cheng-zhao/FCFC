/*******************************************************************************
* benchmark/struct/pca.c: this file is part of the FCFC program.
...
******************************************************************************/
static void tridiag(double x[6], double q[9]) {
  double v, b;
  householder(x + 1, &v, &b);
  double p[2];
  p[0] = b * (x[3] + x[4] * v);
...
******************************************************************************/
static void symmetric_qr(double x[6], double v[9], const double eps) {
...
******************************************************************************/
static inline void sort_eigen(double *x, double *v) {
  if (x[0] < x[3]) {
    if (x[3] < x[5]) {
...
******************************************************************************/
static inline void cov_mat(real **x, const size_t n, double cov[6]) {
  double mean[3];
