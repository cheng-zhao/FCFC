...
Arguments:
  * `ofile`:    structure for writing files;
...
int output_newfile(OFILE *ofile, const char *fname) {
  if (!ofile) {
    P_ERR("the interface for file writing is not initialized\n");
    return FCFC_ERR_ARG;
  }
  if (!fname || !(*fname)) {
...
Function `output_destroy`:
  Deconstruct the interface for writing files.
Arguments:
  * `ofile`:    structure for writing files.
******************************************************************************/
...
Function `output_write`:
  Copy data to the buffer and save it to the file if necessary.
Arguments:
  * `ofile`:    structure for writing files;
...
Return:
  Zero on success; non-zero on error.
******************************************************************************/
int output_write(OFILE *ofile, const void *restrict src, const size_t size) {
  if (!ofile) {
    P_ERR("the interface for file writing is not initialized\n");
    return FCFC_ERR_ARG;
  }
  ...
  if (!src || !size) {
    P_ERR("the data to be written is not given\n");
    return FCFC_ERR_ARG;
  }

  /* Save the data directly to file if it is too large. */
  if (size >= FCFC_MAX_CHUNK) {
    if (output_flush(ofile)) return FCFC_ERR_FILE;
    if (fwrite(src, size, 1, ofile->fp) != 1) {
      P_ERR("failed to write to the output file: `%s'\n", ofile->fname);
      return FCFC_ERR_FILE;
    }
  }
  /* Copy data to the buffer. */
    ...
    /* Check if the buffer is full. */
    if (size + ofile->size > (size_t) ofile->max) {
      bool enlarge = false;
      /* Enlarge the size of the buffer if necessary. */
      while (size > (size_t) ofile->max) {
        if (FCFC_MAX_CHUNK / 2 < ofile->max) ofile->max = size;
        else ofile->max <<= 1;
        enlarge = true;
      }
      ...
      if (enlarge) {
        char *tmp = realloc(ofile->chunk, ofile->max * sizeof(char));
        if (!tmp) {
          P_ERR("failed to allocate memory for saving the data\n");
          return FCFC_ERR_MEMORY;
        }
        ofile->chunk = tmp;
      }
      ...
      if (output_flush(ofile)) return FCFC_ERR_FILE;
      memcpy(ofile->chunk, src, size);
      ofile->size = size;
    }
    else {
      memcpy(ofile->chunk + ofile->size, src, size);
      ofile->size += size;
    }
  }
