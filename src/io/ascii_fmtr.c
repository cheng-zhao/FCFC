...
******************************************************************************/
static inline void print_fmtr_error(const char *fmtr, const char *ptr) {
  fprintf(stderr, "%s\n", fmtr);
  if (ptr > fmtr) {
...
******************************************************************************/
static int arg_insert(asc_arg_s **arg, int *num, const asc_dtype_e dtype,
  ...
  if ((*num & (*num - 1)) == 0) {       /* num is 0 or power of 2 */
    if (INT_MAX / 2 < *num) {
      P_ERR("too many columns in the formatter string\n");
      return FCFC_ERR_CFG;
    }
    size_t size = (*num) ? *num << 1 : 1;       /* double the size */

    tmp = realloc(*arg, size * sizeof(asc_arg_s));
    if (!tmp) {
...
******************************************************************************/
void ascii_arg_destroy(asc_arg_s *arg, const int num) {
  if (!arg) return;
...
******************************************************************************/
asc_arg_s *parse_ascii_fmtr(const char *fmtr, int *num, int *rnum) {
  ...
  while (*fmt) {
    ...
    asc_dtype_e dtype = ASCII_DTYPE_SKIP;
    c = *fmt++;
    switch (c) {
      case 'n':
      case 'd':
  ...
  asc_arg_s *tmp = realloc(args, sizeof(asc_arg_s) * n);
