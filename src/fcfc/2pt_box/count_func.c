...
typedef enum {
  ...
  FCFC_NODE_DIST_X_WRAP         /* need periodic wrap check for each point */
} fcfc_node_dist_e;
...
int count_pairs(const void *tree1, const void *tree2, CF *cf, COUNT *cnt,
  ...
  void *conf = NULL;
  real hsize[3];
  for (int i = 0; i < 3; i++) hsize[i] = cf->bsize[i] * 0.5;
  ...
  /* Initialize the stack for dual nodes. */
  ...
  /* Gather pair counts from different threads and vector elements. */
#ifdef OMP
  if (withwt) {
