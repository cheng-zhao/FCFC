...
******************************************************************************/
static void cnvt_coord_interp(const COORD_CNVT *cnvt, real *x[static 3],
    const size_t ndata) {
#ifdef OMP
  #pragma omp parallel for
#endif
  for (size_t i = 0; i < ndata; i++) {
    double ra = x[0][i] * DEGREE_2_RAD;
    double dec = x[1][i] * DEGREE_2_RAD;
    double dist = sample_interp(cnvt, x[2][i]);

    x[0][i] = dist * cos(dec) * cos(ra);
    x[1][i] = dist * cos(dec) * sin(ra);
    x[2][i] = dist * sin(dec);
  }
}
...
******************************************************************************/
static double *cnvt_z_sample(real *const x[static 3], const size_t ndata,
    const int num) {
...
static int cnvt_legauss_order(const double Omega_m, const double Omega_L,
    ...
    real *const x[static 3], const size_t ndata, const int num) {
  /* Uniformly sample redshifts from the data. */
  double *ztest = cnvt_z_sample(x, ndata, num);
...
static void cnvt_coord_integr(const double Omega_m, const double Omega_L,
    const double Omega_k, const double widx, const int order, real *x[static 3],
    const size_t ndata) {
#ifdef OMP
  #pragma omp parallel for
#endif
  for (size_t i = 0; i < ndata; i++) {
    ...
    double dec = x[1][i] * DEGREE_2_RAD;
    double dist =
      cnvt_legauss(Omega_m, Omega_L, Omega_k, widx, order, x[2][i]);
...
Return:
  Zero on success; non-zero on error.
******************************************************************************/
int cnvt_coord(const CONF *conf, real *x[static 3], const size_t ndata,
    COORD_CNVT *coord) {
  if (!conf) {
  ...
  if (conf->fcnvt) {    /* binary search and cubic spline interpolation */
    ...
    cnvt_coord_interp(coord, x, ndata);
    if (conf->verbose)
    ...
    int order = cnvt_legauss_order(conf->omega_m, conf->omega_l, conf->omega_k,
        widx, conf->ecnvt, x, ndata, FCFC_INT_NUM_ZSP);
    if (order == INT_MAX) {
