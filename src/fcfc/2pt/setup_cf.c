...
void cf_setup_worker(CF **cf, const int rank) {
  ...
  /* Broadcast variables. */
  MPI_Request req[10];
  if (MPI_Ibcast(&c->bintype, 1, MPI_INT, FCFC_MPI_ROOT, MPI_COMM_WORLD, req) ||
      MPI_Ibcast(&c->tabtype, 1, MPI_INT, FCFC_MPI_ROOT, MPI_COMM_WORLD,
          req + 1) ||
      MPI_Ibcast(&c->ns, 1, MPI_INT, FCFC_MPI_ROOT, MPI_COMM_WORLD, req + 2) ||
      MPI_Ibcast(&c->np, 1, MPI_INT, FCFC_MPI_ROOT, MPI_COMM_WORLD, req + 3) ||
      MPI_Ibcast(&c->nmu, 1, MPI_INT, FCFC_MPI_ROOT, MPI_COMM_WORLD, req + 4) ||
  ...
  if (MPI_Ibcast(c->wt, c->npc, MPI_C_BOOL, FCFC_MPI_ROOT, MPI_COMM_WORLD,
      ...
      (c->bintype == FCFC_BIN_SPI && MPI_Ibcast(c->p2bin, c->np + 1,
          FCFC_MPI_REAL, FCFC_MPI_ROOT, MPI_COMM_WORLD, req + nreq++)) ||
      (c->treetype == FCFC_STRUCT_BALLTREE && c->bintype == FCFC_BIN_SPI &&
      MPI_Ibcast(c->pbin, c->np + 1, FCFC_MPI_REAL, FCFC_MPI_ROOT,
          MPI_COMM_WORLD, req + nreq++)) ||
