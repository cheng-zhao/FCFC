...
typedef struct {
        ...
        FCFC_TREE_TYPE  ==  FCFC_STRUCT_BALLTREE
...
#if     FCFC_BIN_TYPE  ==  FCFC_BIN_SPI
  #if   FCFC_TREE_TYPE  ==  FCFC_STRUCT_BALLTREE
  real tsmax;           /* maximum s of interest (i.e. sqrt(s2max + p2max)) */
  real msmax;           /* min{smax, pmax} */
  #else
  real ts2max;          /* maximum squared s of interest (i.e. s2max + p2max) */
  real ms2max;          /* min{s2max, p2max} */
  #endif
  #if   FCFC_BIN_SMIN  ==  FCFC_BIN_MIN_NONZERO  &&                     \
        FCFC_BIN_PMIN  ==  FCFC_BIN_MIN_NONZERO
    #if FCFC_TREE_TYPE  ==  FCFC_STRUCT_BALLTREE
  real tsmin;           /* minimum s of interest (i.e. sqrt(s2min + p2min)) */
    #else
  real ts2min;          /* minimum squared s of interest (i.e. s2min + p2min) */
    #endif
  #endif
  real p2max;
...
static void PRIVATE_NAME(paircnt) (STACK_DUAL_NODE *stack, const void *config,
  ...
  if (node1 == node2) {                 /* the same node */
    ...
    const fcfc_node_dist_e flag =
      PRIVATE_NAME(compare_dual_node) (node1, node2, config);
