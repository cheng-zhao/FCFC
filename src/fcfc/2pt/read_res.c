...
static int read_pair_bin(CNTFILE *cfile, CF *cf, const int idx) {
  ...
  do {
    ...
    while (cfile->size >= sizeof(uint64_t)) {
  ...
  #endif
        if (spec[2] != 0) {
          P_ERR("periodic pair counts in file: `%s'\n", cfile->fname);
          return FCFC_ERR_FILE;
        }
      ...
      else if (nblock == 1) {           /* separation bin edges */
      ...
      else if (nblock == 2 && cf->bintype == FCFC_BIN_SPI) {    /* pi bins */
      ...
      else if (nblock == 2 + off) {     /* number of tracers */
      ...
      else if (nblock == 3 + off) {     /* normalization factor */
        if (size != sizeof(double)) {
          P_ERR("invalid format of pair count file: `%s'\n", cfile->fname);
          return FCFC_ERR_FILE;
        }
      }
      else if (nblock == 4 + off) {     /* normalized pair counts */
      ...
      else if (nblock == 5 + off) {     /* raw pair counts */
